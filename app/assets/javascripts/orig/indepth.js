function factors(filePath) {
  ...
  var color = d3.scale.ordinal().range([colorUsers.total, colorUsers.subscriber, colorUsers.customer]),
    avgs = d3.scale.ordinal().range([colorUsers.total, colorUsers.subscriber, colorUsers.customer]),
    bars = d3.scale.ordinal().range(colorFact),
  ...
  d3.csv("data/" + filePath + ".csv", function(error, data) {
    ...
    color.domain(d3.keys(data[0]).filter(function(key) {
      return (key == "total" || key == "subscriber" || key == "customer");
    }));
    avgs.domain(d3.keys(data[0]).filter(function(key) {
      return (key == "totalAvg" || key == "subscriberAvg" || key == "customerAvg");
    }));
    temps.domain(d3.keys(data[0]).filter(function(key) {
      return (key == "temp");
    }));
    bars.domain(d3.keys(data[0]).filter(function(key) {
      return (key == "weekend" || key == "BART" || key == "49ers" || key == "Giants" || key == "Sharks" || key == "AmericasCup" || key == "rain" || key == "holiday" || key == "USGovShutdown");
    }));
    ...
    y.domain([
      d3.min(userType, function(c) {
        return d3.min(c.values, function(v) {
          return v.rides;
        });
      }),
      d3.max(userType, function(c) {
    ...
    function(error, data) {
      ...
      var heatMap = svg.selectAll(".cell")
        ...
        .on("mouseover", function(d) {
          ...
          d3.selectAll(".rowLabel").classed("text-hover", function(r, ri) {
            return ri == (d.row - 1);
          });
          d3.selectAll(".colLabel").classed("text-hover", function(c, ci) {
            return ci == (d.col - 1);
          });
      ...
      if (city.showlabels) {
        var legend = svg.selectAll(".legend")
          .data([0].concat(colorScale.domain()), function(d) {
            return d;
          })
          .enter().append("g")
          .attr("class", "legend")
...
function horizontalbar(filePath) {
  ...
  d3.csv(filePath, type, function(error, data) {
    ...
    xr.domain([0, d3.max(data, function(d) {
      return d.end;
    })]);
    xl.domain([0, d3.max(data, function(d) {
      return d.start;
    })]);
    ...
    var group = svg.selectAll(".group")
      ...
      .enter().append("g")
      .attr("class", function(d) {
        return d.city + " " + d.transport + " group hover-hide";
      })
      .on("mouseover", function(d) {
        d3.select(this).classed("hover-hide", false);
      })
      .on("mouseout", function(d) {
        d3.select(this).classed("hover-hide", true);
      });
